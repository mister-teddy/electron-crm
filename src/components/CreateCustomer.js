...
export default nojsx(props => {
    ...
    return div(
        Avatar.props({ onClick: () => setVisible(true), style: { backgroundColor: '#1890ff' }, size: 64 })('+'),
        Modal.props({ visible, title: 'Create new customer', onOk, onCancel })(
            Form.props({
                ...
                FormItem.props({ name: 'avatar', label: 'Description ', initialValue: '' })(
                    ...
                    })(imageUrl ? img.props({ src: imageUrl, alt: "avatar", style: { width: '100%' } })() : (loading ? 'Loading' : 'Upload'))
                )
            )
        )
    )
